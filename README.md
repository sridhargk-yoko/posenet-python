...
This repository contains a pure Python implementation (multi-pose only) of the Google TensorFlow.js Posenet model. From https://github.com/rwightman/posenet-python

rwightman first adapted the JS code more or less verbatim and found the performance was low so made some vectorized numpy/scipy version of a few key functions (named `_fast`).
...
rwightman: Development and testing was done with Conda Python 3.6.8 and Tensorflow 1.12.0 on Linux.

rwightman: Windows 10 with the latest (as of 2019-01-19) 64-bit Python 3.7 Anaconda installer was also tested.

sridhargk: Modified and successfully ran the code using ubuntu 20.04 (as of August 2020) using python3 venv

rwightman: If you want to use the webcam demo, a pip version of opencv (`pip install opencv-python`) is required instead of the conda version. Anaconda's default opencv does not include ffpmeg/VideoCapture support. Also, you may have to force install version 3.4.x as 4.x has a broken drawKeypoints binding.

rwightman: A conda environment setup as below should suffice: 
```
...
sridhargk: A python venv environment setup should be suffice:
```
pip install -r requirements.txt

```

sridhargk: `Note: if some of the version missmatched, please use the closer version`
